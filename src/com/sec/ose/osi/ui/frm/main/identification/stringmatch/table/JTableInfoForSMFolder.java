...
/**
 * JTableInfoForSMFolder
 * @author sjh.yoo, hankido.lee
 * 
 */
public class JTableInfoForSMFolder extends JTableMatchedInfoForSM {
	private static Log log = LogFactory.getLog(JTableInfoForSMFolder.class);
	
	private static final long serialVersionUID = 1L;
	protected TableModelForSMFolder tableModelFolder = null;

	public JTableInfoForSMFolder() {
		tableModelFolder = new TableModelForSMFolder();

		this.setSelectionMode(ListSelectionModel.SINGLE_SELECTION); 
		this.setModel(tableModelFolder);
		this.setDefaultRenderer(this.getColumnClass(0),new IdentificationTableRendererForSMFolder());
	...
	public void refresh(ISummaryInfo summaryInfo) {
		...
		log.debug("Update String Match Folder Table - size:"+folderSummary.getStringMatchInforForFolderListSize());
		tableModelFolder.setFolderSummary(folderSummary);
		
		if(folderSummary == null) {
			clear();
			return;
		}
		if(folderSummary.getStringMatchInforForFolderListSize()==0) {
			clear();
			return;
		}	
		
		setModel(tableModelFolder);
		tableModelFolder.setColumnWidth(this);
		
		sorter.setModel(tableModelFolder);
		sorter.toggleSortOrder(TableModelForSMFolder.COL_STATUS);
		this.setRowSorter(sorter);
		this.setRowSelectionInterval(0, 0);
	}

	private void clear() {
		tableModelFolder.setRowCount(0);
		setModel(tableModelFolder);
		tableModelFolder.fireTableDataChanged();
	}
	...
	public String getSelectedSearchName() {
		...
		return (String)this.getValueAt(this.getSelectedRow(),TableModelForSMFolder.COL_STRING_SEARCH);
	}
	
	public String getSelectedLicenseName() {
		if(getSelectedRow() < 0) return null;
		return (String)this.getValueAt(this.getSelectedRow(),TableModelForSMFolder.COL_LICENSE_NAME);
	}
	
	public String getSelectedComponentName() {
		if(getSelectedRow() < 0) return null;
		return (String)this.getValueAt(this.getSelectedRow(),TableModelForSMFolder.COL_COMPONENT_NAME);
	}
}

/**
 * IdentificationTableRendererForSMFolder
 * @author sjh.yoo, hankido.lee
 * 
 */
class IdentificationTableRendererForSMFolder extends DefaultTableCellRenderer {
